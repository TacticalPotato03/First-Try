...
export default function VehicleComparison() {
  ...
  const calc = useMemo(() => {
    ...
    const makeEv = (name, tag, monthlyPay, deposit, depositExtra, consumption, range, dcCharge, insIncluded, separateIns) => {
      ...
      const m = {
        lease: monthlyPay,
        depositAmort: depAmort,
        electricity: elCost,
        insurance,
        roadTax,
        wallbox: wallboxMonthly,
      };
      m.total = m.lease + m.depositAmort + m.electricity + m.insurance + m.roadTax + m.wallbox;

      return {
        name, tag, monthlyPay, deposit, depositExtra, consumption, range, dcCharge,
        insIncluded, monthly: m,
        total48: m.total * months,
        delta: m.total - peugeot.total,
      };
    };

    const pv5 = makeEv("Kia PV5 Upgrade", "Primary Choice",
      4495, 29995, 2500, 19.3, 412, "150 kW", false, pv5Insurance);

    const buzzLife = makeEv("ID. Buzz Lang Life", "Premium Alternative",
      5057, 45000, 0, 20.3, 469, "200 kW", true, 0);

    const buzzStyle = makeEv("ID. Buzz Lang Style", "Premium+",
      6357, 45000, 0, 20.6, 462, "200 kW", true, 0);

    const evs = [pv5, buzzLife, buzzStyle];
    const lowestIdx = evs.reduce((bi, ev, i) => ev.monthly.total < evs[bi].monthly.total ? i : bi, 0);

    const buzzLifeNoIns = buzzLife.monthly.total;
    const pv5NoIns = pv5.monthly.total - pv5Insurance;
    const breakEvenIns = Math.round(buzzLifeNoIns - pv5NoIns);
  ...
  return (
    <div style={{ fontFamily: "'IBM Plex Sans', 'Segoe UI', system-ui, sans-serif" }} className="min-h-screen bg-gray-50 p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        {showExplainer && (
          <Card className="mb-6 p-5 bg-sky-50/60 border-sky-200">
            ...
            <div className="text-sm text-gray-700 space-y-3 leading-relaxed">
              ...
              <p>
                <span className="font-semibold">Play with the sliders</span> in the gray box below to see how the numbers change. The most interesting ones to try:
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs mt-2">
                <div className="bg-white rounded-lg p-2.5 border border-sky-100">
                  <span className="font-semibold">PV5 insurance:</span> We estimated 650 DKK/mo since it's not included in the Kia lease. If we get a real quote that's lower, the Kia gets even cheaper.
                </div>
                <div className="bg-white rounded-lg p-2.5 border border-sky-100">
                  <span className="font-semibold">Repair trend:</span> "Rising +15%" assumes the Peugeot's repair costs keep going up. "Stable" uses the last 3 years as-is. Either way, it's a lot.
                </div>
                <div className="bg-white rounded-lg p-2.5 border border-sky-100">
                  <span className="font-semibold">Annual km:</span> More driving makes EVs look better (electricity is much cheaper than diesel per km).
                </div>
                <div className="bg-white rounded-lg p-2.5 border border-sky-100">
                  <span className="font-semibold">Electricity price:</span> Denmark cut the electricity tax in 2026, so 2.10 DKK/kWh is a reasonable starting point for home charging.
                </div>
              </div>
              <p className="text-xs text-gray-500 mt-2">
                The bottom line: both EVs cost more per month than the Peugeot's "average," but the Peugeot's average hides huge swings. In 2023 and 2025, repairs alone were over 43,000 DKK each year. An EV lease converts that unpredictable expense into a fixed monthly payment.
              </p>
            </div>
          </Card>
        )}
        ...
        <Card className="mb-6 p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            {[
              { label: "Annual km", min: 15000, max: 35000, step: 1000, val: annualKm, set: setAnnualKm, fmt: v => v.toLocaleString("da-DK") + " km" },
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-gray-100">
            <div>
              <label className="block text-xs text-gray-500 mb-1">
                PV5 insurance (DKK/mo)
                <InfoTip text="The Kia lease excludes insurance. Adjust based on actual quotes." />
              </label>
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">
                Repair trend
                <InfoTip text="'Rising' adds 15% to the 3-year avg. 2 of 3 full years exceeded 43k DKK." />
              </label>
              <div className="flex gap-2 mt-2">
                {["stable", "rising"].map(v => (
                  <button key={v} onClick={() => setRepairTrend(v)}
                    className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${
                      repairTrend === v ? "bg-gray-800 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                    }`}>{v === "rising" ? "Rising +15%" : "Stable"}</button>
                ))}
              </div>
            </div>
        ...
        {/* Main 4-column comparison */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          {/* Peugeot */}
          <Card className="p-5 border-2 border-orange-200 bg-orange-50/40">
            ...
            <p className="text-xs text-gray-400 mb-5">2013 · 1.6 HDi Diesel · Auto</p>
            <div className="text-4xl font-extrabold text-gray-900 tracking-tight">{formatDKK(Math.round(calc.peugeot.total))}</div>
            <p className="text-xs text-gray-400 mb-5">per month · all-in</p>
            <div className="space-y-2.5 text-xs border-t border-orange-200 pt-4">
              {[["Fuel", calc.peugeot.fuel], ["Insurance", calc.peugeot.insurance], ["Road tax", calc.peugeot.roadTax]].map(([l, v]) => (
                <div key={l} className="flex justify-between">
                  <span className="text-gray-500">{l}</span>
                  <span className="font-medium text-gray-700">{formatDKK(Math.round(v))}</span>
                </div>
              ))}
              <div className="flex justify-between">
                <span className="text-gray-500">Repairs (avg.)</span>
                <span className="font-semibold text-orange-600">{formatDKK(Math.round(calc.peugeot.repairs))}</span>
              </div>
            </div>
            <div className="mt-5 pt-4 border-t border-orange-200 space-y-1 text-xs">
          ...
          {calc.evs.map((ev, i) => {
            ...
            return (
              <Card key={ev.name} className={`p-5 ${isBest ? `border-2 ${accentBorder[i]} ${accentBg[i]}/40` : ""}`}>
                <span className={`text-xs font-semibold uppercase tracking-wider ${accentText[i]}`}>
                  {isBest ? "Lowest All-In" : ev.tag}
                </span>
                <h3 className="font-bold text-gray-900 mt-1 leading-tight">{ev.name}</h3>
                <p className="text-xs text-gray-400 mb-5">{ev.range} km · {ev.consumption} kWh/100km · {ev.dcCharge} DC</p>

                <div className="text-4xl font-extrabold text-gray-900 tracking-tight">{formatDKK(Math.round(ev.monthly.total))}</div>
                <p className="text-xs text-gray-400">per month · all-in incl. deposit</p>
                <p className={`text-xs font-bold mt-1 mb-5 ${ev.delta > 0 ? "text-red-500" : "text-emerald-600"}`}>
                  {ev.delta > 0 ? "+" : ""}{formatDKK(Math.round(ev.delta))} vs. Peugeot
                </p>

                <div className="space-y-2.5 text-xs border-t border-gray-200 pt-4">
                  <div className="flex justify-between">
                    <span className="text-gray-500">Lease {ev.insIncluded ? "(incl. insurance)" : ""}</span>
                    <span className="font-medium">{formatDKK(ev.monthlyPay)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Deposit (amort.) <InfoTip text={`${formatDKK(ev.deposit + ev.depositExtra)} upfront over 48 months`} /></span>
                    <span className="font-medium">{formatDKK(Math.round(ev.monthly.depositAmort))}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Electricity</span>
                    <span className="font-medium text-emerald-600">{formatDKK(Math.round(ev.monthly.electricity))}</span>
                  </div>
                  {!ev.insIncluded && (
                    <div className="flex justify-between">
                      <span className="text-gray-500">Insurance <InfoTip text="Not included in Kia lease. Adjust with slider above." /></span>
                      <span className="font-medium text-blue-600">{formatDKK(ev.monthly.insurance)}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                    <span className="text-gray-500">Road tax</span>
                    <span className="font-medium">{formatDKK(Math.round(ev.monthly.roadTax))}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Wallbox (amort.)</span>
                    <span className="font-medium">{formatDKK(Math.round(ev.monthly.wallbox))}</span>
                  </div>
                </div>

                <div className="mt-5 pt-4 border-t border-gray-200 text-xs space-y-1">
        ...
        <Card className="mb-6 p-4 bg-blue-50/50 border-blue-100">
          <div className="flex gap-3 items-start">
            ...
            <div className="text-sm text-gray-700">
              ...
              <p>
                PV5: <span className="font-bold">{formatDKK(32495)}</span> (29,995 + 2,500 depositum) · Buzz Life/Style: <span className="font-bold">{formatDKK(45000)}</span> · Difference: <span className="font-bold">{formatDKK(12505)}</span> less with the Kia.
              </p>
            </div>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            {[
              ...
              <div key={r.label} className="mb-3">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="font-semibold">{formatDKK(Math.round(r.val))}/mo</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2.5">
                  <div className={`${r.color} h-2.5 rounded-full transition-all`}
                    style={{ width: `${Math.min(100, (r.val / Math.max(calc.peugeot.fuel, 1)) * 100)}%` }}></div>
                </div>
                <p className="text-xs text-gray-400 mt-0.5">{r.sub}</p>
              </div>
            ))}
            <div className="mt-3 pt-3 border-t border-gray-100 text-xs text-gray-500">
              Monthly energy saving: <span className="font-bold text-emerald-600">{formatDKK(Math.round(calc.peugeot.fuel - calc.evs[0].monthly.electricity))}</span>
            </div>
          </Card>
          ...
          <Card className="p-5">
            ...
            {[
              { year: "2023", cost: 43972 },
              { year: "2024", cost: 15353 },
              { year: "2025", cost: 45178 },
              { year: "2026 (YTD Mar)", cost: 17155 },
            ].map(r => (
              <div key={r.year} className="mb-3">
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-gray-600">{r.year}</span>
                  <span className="font-semibold">{formatDKK(r.cost)}</span>
                </div>
                <div className="w-full bg-gray-100 rounded-full h-2.5">
                  <div className={`h-2.5 rounded-full ${r.cost > 35000 ? "bg-red-400" : "bg-orange-300"}`}
                    style={{ width: `${Math.min(100, (r.cost / 50000) * 100)}%` }}></div>
                </div>
              </div>
            ))}
            <div className="mt-3 pt-3 border-t border-gray-100 text-xs text-gray-500">
            ...
            {[
              ...
              <div key={item.n} className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 leading-relaxed">{item.b}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card className="p-5 bg-gray-900 text-white border-gray-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 text-sm">
            {[
              "Second-row comfort: 3 × 180cm teenagers side-by-side",
              "Sälen winter trip: 800 km in sub-zero, charging stops",
              "Tow hitch availability and towing capacity",
              "Lease vs. Buy: depreciation hedge vs. equity",
              "Kia PV5 delivery timeline and early reviews",
              "DC charger network on E45/E6 corridor to Sälen",
            ].map((item, i) => (
        ...
        <p className="text-xs text-gray-400 mt-4 text-center">
          Diesel {dieselPrice.toFixed(1)} DKK/L · Peugeot 6.5 L/100km · Electricity {elPrice.toFixed(1)} DKK/kWh · EV road tax 920 DKK/yr · All leases 48 mo / 30,000 km/yr
        </p>
      </div>
    </div>
  );
